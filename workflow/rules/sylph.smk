...
rule prok_sylph:
    input:
        sr1=expand(os.path.join(DATA_DIR, "reads/{sid}_filtered.R1.fq"), sid=SAMPLES.index),
        sr2=expand(os.path.join(DATA_DIR, "reads/{sid}_filtered.R2.fq"), sid=SAMPLES.index),
        gtdb=os.path.join(DB_DIR, "sylph/gtdb-r220-c200-dbv1.syldb")
    output:
