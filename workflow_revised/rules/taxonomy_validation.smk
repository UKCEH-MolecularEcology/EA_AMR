...
rule compare_taxonomy_methods:
    input:
        ...
        # bracken_reads_c05 and _c07 inputs are optional — only included when
        # bracken_highconf step has been run (reads needed, see bracken_reads.smk).
        # Paths passed as params below; files are loaded with try/except.

        bracken_contigs=os.path.join(RESULTS_DIR, "bracken/contig/combined_bracken.txt"),
        bracken_contigs_2kb=os.path.join(RESULTS_DIR, "bracken/contig_2kb/combined_bracken.txt"),
        bracken_contigs_10kb=os.path.join(RESULTS_DIR, "bracken/contig_10kb/combined_bracken.txt"),
        singlem=config.get("singlem_results", {}).get(
            "combined_relab",
    ...
    run:
        ...
        for rank_name, rank_prefix in rank_items:
            ...
            c05_path = os.path.join(RESULTS_DIR, "bracken_c05/combined_bracken_c05.txt")
            c07_path = os.path.join(RESULTS_DIR, "bracken_c07/combined_bracken_c07.txt")

            dfs = {
                "Kraken2_reads_c02":    parse_combined_bracken(str(input.bracken_reads),        rank_prefix),
            ...
            if os.path.exists(c05_path):
                dfs["Kraken2_reads_c05"] = parse_combined_bracken(c05_path, rank_prefix)
            if os.path.exists(c07_path):
                dfs["Kraken2_reads_c07"] = parse_combined_bracken(c07_path, rank_prefix)
